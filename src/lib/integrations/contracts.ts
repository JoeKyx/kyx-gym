...
export const exerciseSchema = z
  ...
  .strict();
export const cardioPhaseSchema = z
  .object({
    name: text,
    duration_seconds: z.number().int().min(1).max(86400),
    speed_kmh: z.number().finite().positive().max(100).nullable(),
    distance_km: z.number().finite().positive().max(1000).nullable(),
  })
  .strict();
export const cardioSessionSchema = z
  .object({
    name: text,
    activity: z.enum(['running', 'cycling', 'rowing', 'walking', 'other']),
    status: z.enum(['planned', 'completed']),
    performed_at: z.string().datetime({ offset: true }).nullable(),
    rounds: z.number().int().min(1).max(100),
    phases: z.array(cardioPhaseSchema).min(1).max(20),
    notes: z.string().trim().max(2000),
  })
  .strict();
export type CardioSessionInput = z.infer<typeof cardioSessionSchema>;
export type CardioSession = CardioSessionInput & {
  id: string;
  created_at: string;
};
...
export const inputs = {
  list_cardio_sessions: z
    .object({
      after: z.string().uuid().optional(),
      limit: z.number().int().min(1).max(50).default(20),
    })
    .strict(),
  get_cardio_session: z.object({ id: z.string().uuid() }).strict(),
  create_cardio_session: z
    .object({ request_id: z.string().uuid(), session: cardioSessionSchema })
    .strict(),
  list_workouts: z
    .object({
      ...page,
...
export const toolScopes: Partial<Record<Operation, Scope>> = {
  list_cardio_sessions: 'training.read',
  get_cardio_session: 'training.read',
  create_cardio_session: 'plans.write',
